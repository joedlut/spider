...
# 原来的数据是 sqlite3的，那么目标的系统用的也必须是sqlite3， 不然执行 python manage.py inspectdb 看不到输出的数据

# 因为使用的数据有了， 不用执行 python manage.py makemigrations   python manage.py migrate

#直接执行 python runserver 0.0.0.0:8000
...
class Candidate(models.Model):
    ...
    first_interviewer_user = models.ForeignKey(User, on_delete=models.DO_NOTHING, related_name='first_interviewer_user' ,blank=True, null=True)
    hr_interviewer_user = models.ForeignKey(User, on_delete=models.DO_NOTHING, blank=True, related_name='hr_interviewer_user',null=True)
    second_interviewer_user = models.ForeignKey(User, on_delete=models.DO_NOTHING, blank=True,related_name='second_interviewer_user', null=True)
